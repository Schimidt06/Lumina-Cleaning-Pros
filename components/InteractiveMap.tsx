...
export const InteractiveMap: React.FC<InteractiveMapProps> = ({ darkMode, lang, onBookNow }) => {
  ...
  return (
    <div className={`relative w-full rounded-xl sm:rounded-[2rem] md:rounded-[3rem] overflow-hidden border transition-all duration-500 flex flex-col ${darkMode ? 'bg-[#080c17] border-white/10 shadow-2xl' : 'bg-white border-slate-200 shadow-2xl shadow-slate-200/50'
      }`}>
...
>>>>>>> 0ecfa5ab331dce1c386264a95e98f295f89583c4
        ...
        <div className="relative w-full md:w-72">
          ...
          <input
            ...
            className={`w-full h-14 rounded-xl pl-12 pr-10 outline-none border-2 transition-all text-sm font-bold ${darkMode
            ...
            {hasInput && !isDebouncing ? (
              hasResults ? (
                filteredResults.map((result, i) => (
                  <button
                    ...
                    className={`w-full p-4 rounded-xl border-2 transition-all flex items-center justify-between text-left group ${darkMode ? 'bg-white/5 border-transparent hover:border-blue-500/30' : 'bg-white border-slate-50 hover:border-blue-100'
                      }`}
                  >
                    <div>
              ...
              SERVICE_AREAS.map((area) => (
                <button
                  ...
                  className={`w-full text-left p-5 rounded-2xl border-2 transition-all duration-300 flex items-center justify-between group ${activeArea === area.state && !hasInput
...
>>>>>>> 0ecfa5ab331dce1c386264a95e98f295f89583c4
            <div className="space-y-4">
              ...
              <div>
                ...
                <h5 className={`text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-black tracking-tighter leading-[0.9] ${darkMode ? 'text-white' : 'text-slate-900'}`}>
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {currentAreaObj?.cities.map((city) => (
                <span key={city} className={`px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest border-2 transition-all ${hasInput && city.toLowerCase().includes(searchQuery.toLowerCase())
            ...
            <div className={`p-6 rounded-[2rem] border-2 text-left flex gap-5 items-start ${darkMode ? 'bg-white/[0.03] border-white/5' : 'bg-white border-slate-100 shadow-md'
              }`}>
